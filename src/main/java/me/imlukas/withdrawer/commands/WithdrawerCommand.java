    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            if (!player.hasPermission("withdrawer.admin")) {
                ...
                this.messages.sendStringMessage(player, "&7/withdrawer toggleactionbar");
                this.messages.sendStringMessage(player, "&7/withdrawer toggleintrusive");
                this.messages.sendStringMessage(player, "&7/withdrawer toggleprefix");
                return true;
            }
            if (player.hasPermission("withdrawer.admin")) {
                this.messages.sendStringMessage(player, "&7||---- &dWithdrawer &7----||");
                this.messages.sendStringMessage(player, "&7Version: V1.5");
                ...
                this.messages.sendStringMessage(player, "- &7/withdrawer help - see all the commands");
                this.messages.sendStringMessage(player, "- &7/withdrawer toggleactionbar - toggle the actionbar messages");
                this.messages.sendStringMessage(player, "- &7/withdrawer toggleintrusive - toggle between less intrusive messages");
                this.messages.sendStringMessage(player, "- &7/withdrawer toggleprefix - deactivate/activate prefix on messages");
                return true;
            }
        }

        String action = args[0];
        if (action.equalsIgnoreCase("toggleintrusive")){
